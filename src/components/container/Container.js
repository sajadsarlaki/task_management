...
const tempStatuses = JSON.parse(
  localStorage.getItem('columns') || JSON.stringify(statuses)
);
...
const Container = () => {
  ...
  const updateItem = (updatedItem) => {
    ...
    for (let i = 0; i < items.length; i++) {
      if (items[i].id === updatedItem.id) index = i;
    }
    ...
    <div className="content">
      ...
      <div className="col-wrapper">
        <input
          ...
          onChange={(e) =>
            setNewColumn({ ...newColumn, status: e.target.value })
          }
