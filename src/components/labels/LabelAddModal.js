...
const LabelAddModal = ({
  ...
  const selectColor = (e, color) => {
    const spans = document.getElementsByClassName(
      'add-label-modal__single-color'
    );
    for (let span of spans) span.innerText = '';
    e.target.innerText = '✔';
  ...
  return (
    ...
    >
      <div className={'add-label-modal__header'}>
        <p
          onClick={onLabelModalClosed}
          className={'add-label-modal__header-txt'}
        >
          Add label
        </p>
        <span
          className={'add-label-modal--action add-label-modal__cancel'}
          onClick={onLabelModalClosed}
        >
          ❌
        </span>
      ...
      <div>
        <span
          className={'add-label-modal--add-btn'}
        ...
        {!mode ? (
          <span className={'add-label-modal--dl-btn'} onClick={handleDelete}>
            remove
          </span>
        ) : (
          ''
